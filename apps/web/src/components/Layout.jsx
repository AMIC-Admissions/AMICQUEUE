...
const Layout = ({ children, hideHeader = false, hideFooter = false }) => {
  ...
  return (
    <div className="relative min-h-[100dvh] flex flex-col w-full overflow-x-hidden bg-background" dir={language === 'ar' ? 'rtl' : 'ltr'}>
      {/* Background Image from settings with Light Transparency Overlay */}
      {settings?.backgroundImage ? (
        <div 
          className="fixed inset-0 z-0 bg-cover bg-center bg-no-repeat opacity-20 pointer-events-none"
          style={{ backgroundImage: `url(${pb.files.getUrl(settings, settings.backgroundImage)})` }}
        />
      ) : (
        <div className="fixed inset-0 z-0 bg-office opacity-[0.15] pointer-events-none mix-blend-multiply" />
      )}
      
      <div className="fixed inset-0 z-0 bg-gradient-to-b from-background/80 to-background pointer-events-none" />
