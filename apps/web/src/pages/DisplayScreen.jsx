...
const formatBranch = (branch, language = 'en') => getBranchLabel(branch, language) || branch || 'Branch';
...
const formatTime = (date, language) => new Intl.DateTimeFormat(language === 'ar' ? 'ar-SA' : 'en-US', {
  hour: '2-digit',
  minute: '2-digit',
}).format(date);

const formatDate = (date, language) => new Intl.DateTimeFormat(language === 'ar' ? 'ar-SA' : 'en-US', {
  weekday: 'short',
  day: 'numeric',
  month: 'long',
  year: 'numeric',
...
const DisplayScreenContent = () => {
  ...
  const displayT = t.display || {};
  const commonT = t.common || {};
  const serviceT = t.services || {};
  const branchT = t.branches || {};
  const isArabic = language === 'ar';
  const requestedBranch = normalizeBranch(params?.branchKey || searchParams.get('branch') || '', '');
  const activeBranchCode = requestedBranch || null;
  const activeBranchLabel = activeBranchCode ? (branchT[activeBranchCode] || getBranchLabel(activeBranchCode, language)) : null;
  ...
  useEffect(() => {
    const fetchTickets = async () => {
      try {
        const filter = activeBranchCode
          ? `status="Called" && branch="${activeBranchCode}"`
          : 'status="Called"';
  ...
  }, [activeBranchCode, addToVoiceQueue, lastAnnouncedTicket]);
  ...
  const calledTickets = useMemo(
    () => [...tickets]
      .filter((ticket) => !activeBranchCode || normalizeBranch(ticket?.branch) === activeBranchCode)
      .sort((left, right) => new Date(getTicketTimestamp(right)) - new Date(getTicketTimestamp(left))),
    [tickets, activeBranchCode],
  );
  ...
  const getLocalizedBranch = (branch) => branchT[branch] || formatBranch(branch, language);
  ...
  return (
    ...
    >
      <Helmet><title>{`${displayT.pageTitle || 'Queue Display'}${activeBranchLabel ? ` - ${activeBranchLabel}` : ''} - AMIC`}</title></Helmet>
        ...
        <header className="grid grid-cols-[auto_1fr_auto] items-start gap-6">
          <div className="flex items-start">
            <div className="rounded-[28px] border border-[#222D64]/10 bg-white/92 px-5 py-4 shadow-[0_18px_40px_rgba(34,45,100,0.08)]">
              <img src={logoUrl} alt="AMIC group logo" className="h-12 w-auto object-contain xl:h-14" />
            </div>
          </div>

          <div className="flex flex-col items-center pt-1 text-center">
            <p className="text-sm font-bold uppercase tracking-[0.35em] text-[#222D64]/45">
              {displayT.systemName || 'AMIC Queue System'}
            </p>
            ...
            <div className="mt-4 inline-flex items-center rounded-full border border-[#222D64]/10 bg-white/86 px-4 py-2 text-sm font-semibold text-[#222D64]/65 shadow-sm">
              {activeBranchLabel ? `${displayT.liveBadge || 'Live public display'} - ${activeBranchLabel}` : (displayT.liveBadge || 'Live public display')}
            </div>
          </div>
          ...
          <div className="flex flex-col items-end gap-3">
            ...
            <div className="min-w-[220px] rounded-[24px] border border-[#222D64]/10 bg-white/88 px-5 py-4 text-right shadow-[0_16px_32px_rgba(34,45,100,0.06)]">
              <div className="flex items-center justify-end gap-2 text-xs font-bold uppercase tracking-[0.28em] text-[#222D64]/45">
                ...
                {commonT.today || displayT.today || 'Today'}
              </div>
                  ...
                  <div className="mt-10 grid gap-6 xl:grid-cols-[1.35fr_0.95fr]">
                    <div className="rounded-[34px] bg-[#222D64] p-8 text-white shadow-[0_24px_60px_rgba(34,45,100,0.28)] xl:p-10">
                      <p className="text-sm font-bold uppercase tracking-[0.32em] text-white/58">
                        {displayT.ticketNumber || 'Ticket Number'}
                      </p>
                      ...
                      <div className="mt-8 flex flex-wrap gap-3">
                        <span className="rounded-full bg-white/10 px-4 py-2 text-base font-semibold">
                          {getLocalizedBranch(currentTicket.branch)}
                        </span>
                        <span className="rounded-full bg-white/10 px-4 py-2 text-base font-semibold">
                          {getLocalizedService(currentTicket.service)}
                        </span>
                      </div>
                    </div>
                      ...
                      <div className="mt-8 grid gap-4 sm:grid-cols-2">
                        <div className="rounded-[24px] bg-[#f5f7fb] px-5 py-4">
                          <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-[0.24em] text-[#222D64]/42">
                            ...
                            {displayT.branch || commonT.branch || 'Branch'}
                          </div>
                        ...
                        <div className="rounded-[24px] bg-[#f5f7fb] px-5 py-4">
                          <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-[0.24em] text-[#222D64]/42">
                            ...
                            {displayT.calledAt || 'Called At'}
                          </div>
                          <p className="mt-3 text-2xl font-black text-[#222D64]">
                            {formatTime(new Date(getTicketTimestamp(currentTicket)), language)}
                          </p>
                        </div>
                      </div>
                    </div>
              ...
              >
                <div className="rounded-[42px] border border-[#222D64]/10 bg-white/82 px-10 py-16 text-center shadow-[0_26px_72px_rgba(34,45,100,0.12)] backdrop-blur-xl">
                  ...
                  <h2 className="mt-8 text-5xl font-black tracking-tight text-[#222D64] xl:text-6xl">
                    {displayT.queueStandby || 'Queue standby'}
                  </h2>
                  <p className="mx-auto mt-5 max-w-3xl text-xl font-medium leading-9 text-[#222D64]/58 xl:text-2xl">
                    {displayT.queueStandbyDescription || 'The next called ticket will appear here immediately with its service and counter number.'}
                  </p>

                  {fetchError ? (
                    <p className="mt-6 text-base font-bold text-amber-700">
                      {displayT.retrying || 'Live connection is retrying.'}
                    </p>
                  ) : null}
                </div>
              </motion.section>
        ...
        <footer className="grid items-start gap-4 xl:grid-cols-[260px_1fr]">
          <div className="rounded-[28px] bg-[#222D64] px-6 py-5 text-white shadow-[0_20px_50px_rgba(34,45,100,0.24)]">
            <p className="text-xs font-bold uppercase tracking-[0.32em] text-white/55">
              {displayT.recentCalls || 'Recent Calls'}
            </p>
            ...
            {previousTickets.length > 0 ? previousTickets.map((ticket) => (
              ...
              >
                ...
                <div className="mt-5 flex items-center justify-between text-sm font-semibold text-[#222D64]/58">
                  <span>{getLocalizedBranch(ticket.branch)}</span>
                  <span>{`${displayT.counter || commonT.counter || 'Counter'} ${getCounterValue(ticket)}`}</span>
                </div>
              </div>
            )) : (
              <div className="rounded-[28px] border border-dashed border-[#222D64]/15 bg-white/68 px-6 py-8 text-lg font-semibold text-[#222D64]/48 xl:col-span-4">
                {displayT.noRecentCalls || 'No recent calls yet.'}
              </div>
            )}
          </div>
        </footer>
